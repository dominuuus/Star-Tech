...
const mapRawGreenMissionChallengeToGreenMissionChallenge = (raw: RawGreenMissionLevel): GreenMissionChallenge => ({
    id: raw.id,
    nome: raw.nome,
...
export async function fetchGreenMissionLevel(): Promise<GreenMissionChallenge[]> {
  try {
    ...
    return rawGreenMissions.map(mapRawGreenMissionChallengeToGreenMissionChallenge);
  } catch (error) {
    console.error("Erro ao buscar missoes_verdes via API:", error);
    try {
      const rawGreenMissions: RawGreenMissionLevel[] = gameData.desafios_missoes_verdes;
      if (!Array.isArray(rawGreenMissions)) {
